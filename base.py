...
#         \
#  2. removed the _outdated_save function in baseconcatdataset\
##################################################################\
\
import os\
import json\
...
\
    ...
    \
    The `metadata` attribute of the Epochs\
...
\
    The `metadata` attribute must contain a column called `target`, which will be used to return\
    the target that corresponds to a window. Additional columns\
    `i_window_in_trial`, `i_start_in_trial`, `i_stop_in_trial` are also\
    required to serve information about the windowing (e.g., useful for cropped\
    training).\
